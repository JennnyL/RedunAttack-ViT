...
def set_seed(seed: int = 41):
    ...
    # 确保 cudnn 使用确定性算法（速度可能会变慢）
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


def split_list_4fold(lst):
    n = len(lst)
    chunk_size = (n + 3) // 4  # 向上取整，避免漏元素
    return [lst[i * chunk_size : (i + 1) * chunk_size] for i in range(4)]
...
def get_parser():
    ...
    parser.add_argument(
        "--eval_model", default="llava", type=str, help="the source surrogate model"
    )
    parser.add_argument(
        "--input_dir",
        default="./data",
...
def eval_llava(prompt, image_file: str, others=None):
    assert image_file != ""
...
def eval_llm(llm_func, dataloader, max_option_num=1000):
    folder_path = dataloader.dataset.data_dir
    correct, total = 0, 0
    others = None
...
def main():
    ...
    if args.eval_model == "llava":
        max_option_num = 350
    llm_func = llm_func_map[args.eval_model]
    ...
    acc = eval_llm(llm_func, dataloader, max_option_num)
    res = f"Eval {args.output_dir} on {args.eval_model}, attack success rate: {acc:.1f}"
    print(res)

    with open("eval_llm_res.log", "a") as fl:
        fl.write("\n")
        fl.write(res)
        fl.write("\n")
